      ...
      <div className="w-full max-w-7xl px-4 sm:px-6 md:px-10 transform scale-90 sm:scale-95 md:scale-100 origin-top">
        ...
        <div className="text-center mb-10 sm:mb-16">
          <motion.h2
            ...
            viewport={{ once: true }}
            variants={fadeUp}
            className="text-2xl sm:text-4xl md:text-5xl font-normal font-henju text-white"
          >
          ...
          >
            Real stories from real patients—see how personalized treatment made a
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {testimonials.map((t, i) => (
            <motion.div
              ...
              viewport={{ once: true }}
              variants={slideRight}
              className="bg-[#0D5159] rounded-xl px-6 py-6 flex flex-col items-center text-center text-white 
                         transition-transform duration-300 hover:scale-105 hover:shadow-2xl
                         w-full h-auto min-h-[350px]"
            >
              {/* Patient Images */}
              <div
                className={`grid ${
              ...
              {/* Testimonial Text */}
              <p className="text-xs sm:text-sm md:text-base leading-relaxed mb-3 line-clamp-6">
                “{t.text}”
              </p>
              ...
              <div className="flex items-center justify-center gap-2 mt-2">
                <p className="text-xs sm:text-sm md:text-base font-semibold">{t.name}</p>
                <span className="text-[10px] sm:text-xs md:text-sm border border-[#0C7885]  text-orange-400 px-2 py-1 rounded-full">
                  {t.tag}
                </span>
              </div>
            </motion.div>
